...
static READ_HANDLER(dcon_background_r)
{
    return READ_WORD(&dcon_back_data[offset]);
}
static READ_HANDLER(dcon_foreground_r)
{
    return READ_WORD(&dcon_fore_data[offset]);
}
static READ_HANDLER(dcon_midground_r)
{
    return READ_WORD(&dcon_mid_data[offset]);
}

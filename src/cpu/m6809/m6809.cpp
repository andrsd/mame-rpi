...
void m6809_set_reg(int regnum, unsigned val)
{
    ...
    case M6809_NMI_STATE:
        ...
        break;
    case M6809_IRQ_STATE:
        m6809.irq_state[M6809_IRQ_LINE] = val;
        break;
    case M6809_FIRQ_STATE:
        m6809.irq_state[M6809_FIRQ_LINE] = val;
        break;
    default:
        if (regnum <= REG_SP_CONTENTS) {
            unsigned offset = S + 2 * (REG_SP_CONTENTS - regnum);
...
{
    state_save(file, "m6809");
}
void m6809_state_load(void *file)
{
    state_load(file, "m6809");
}
...
l_0x88:
    ...
    EA = X + SIGNED(EA);
    m6809_ICount -= 1;
    return; /* this is a hack to make Vectrex work. It should be m6809_ICount-=1. Dunno where the cycle was lost :( */
l_0x89:
    IMMWORD(ea);
    EA += X;
    m6809_ICount -= 4;
...
l_0x90:
    ...
    EAD = RM16(EAD);
    m6809_ICount -= 5;
    return; /* Indirect ,R+ not in my specs */
l_0x91:
    EA = X;
    X += 2;
    EAD = RM16(EAD);
...
{
    state_save(file, "hd6309");
}
void hd6309_state_load(void *file)
{
    state_load(file, "hd6309");
}

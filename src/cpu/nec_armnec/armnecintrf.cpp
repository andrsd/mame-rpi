...
static int armnec_UnrecognizedCallback(unsigned int a)
{
    return 0;
}
...
void armnec_set_irq_callback(int (*callback)(int irqline))
{
    ARMNEC.irq_callback = callback;
}
unsigned armnec_dasm(char *buffer, unsigned pc)
{
...
unsigned armnec_get_reg(int regnum)
{
    ...
    switch (regnum) {
    case ARMNEC_IP:
        return ARMNEC.regs.ip;
    case ARMNEC_FLAGS:
        return ARMNEC.regs.flags;
    case ARMNEC_AW:
        return regs[0];
    case ARMNEC_CW:
        return regs[1];
    ...
    case ARMNEC_IY:
        return regs[7];
    case ARMNEC_ES:
        return ARMNEC.regs.sreg[0];
    case ARMNEC_CS:
        return ARMNEC.regs.sreg[1];
    case ARMNEC_SS:
        return ARMNEC.regs.sreg[2];
    case ARMNEC_DS:
        return ARMNEC.regs.sreg[3];
    case ARMNEC_VECTOR:
        return ARMNEC.int_vector;
    case ARMNEC_PENDING:
        return ARMNEC.pending_irq;
    case ARMNEC_NMI_STATE:
        return ARMNEC.nmi_state;
    case ARMNEC_IRQ_STATE:
        return ARMNEC.irq_state;
    case REG_PREVIOUSPC:
...
void armnec_set_reg(int regnum, unsigned val)
{
    ...
    case ARMNEC_IY:
        ...
        break;
    case ARMNEC_ES:
        ARMNEC.regs.sreg[0] = val;
        break;
    case ARMNEC_CS:
        ARMNEC.regs.sreg[1] = val;
        break;
    case ARMNEC_SS:
        ARMNEC.regs.sreg[2] = val;
        break;
    case ARMNEC_DS:
        ARMNEC.regs.sreg[3] = val;
        break;
    case ARMNEC_VECTOR:
        ARMNEC.int_vector = val;
        break;
    case ARMNEC_PENDING:
        ARMNEC.pending_irq = val;
        break;
    case ARMNEC_NMI_STATE:
        armnec_set_nmi_line(val);
        break;
    case ARMNEC_IRQ_STATE:
        armnec_set_irq_line(0, val);
        break;
    default:
        if (regnum <= REG_SP_CONTENTS) {
            unsigned offset = armnec_get_sp() + 2 * (REG_SP_CONTENTS - regnum);
...
const char *armv30_info(void *context, int regnum)
{
    ...
    case CPU_INFO_CREDITS:
        return "ArmNec emulator by Oopsware";
    }
    return "";
}

...
static void gles2_DrawQuad_8(const ShaderInfo *sh, GLuint p_textures[2])
{
    ...
    SHOW_ERROR
    glBindTexture(GL_TEXTURE_2D, p_textures[1]);
    SHOW_ERROR
...
void gles2_draw(void *screen, int width, int height, int depth)
{
    ...
    if (depth == 8 && palette_changed) {
        ...
        glBindTexture(GL_TEXTURE_2D, textures[1]);
        SHOW_ERROR
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 256, 1, GL_RGB, GL_UNSIGNED_SHORT_5_6_5, (const GLvoid*) gp2x_palette);
        SHOW_ERROR
        palette_changed = 0;
    }
    ...
    if (depth == 8) {
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width, height, GL_LUMINANCE, GL_UNSIGNED_BYTE, (unsigned char*) screen);
        SHOW_ERROR
        gles2_DrawQuad_8(&shader, textures);
    } else {
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width, height, GL_RGB, GL_UNSIGNED_SHORT_5_6_5, (unsigned short*) screen);
        SHOW_ERROR

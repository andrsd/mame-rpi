...
WRITE_HANDLER(m92_vram_w)
{
    ...
    } else if (a == pf1_vram_ptr || (a == pf1_vram_ptr + 0x4000)) {
        tilemap_mark_tile_dirty(pf1_layer, offset / 4);
...
WRITE_HANDLER(m92_master_control_w)
{
    ...
    case 0: /* Playfield 1 (top layer) */
        if ((pf4_control[0] & 0x10) == 0x10) pf1_enable = 0;
        else pf1_enable = 1;
        if ((pf4_control[0] & 0x40) == 0x40) pf1_rowscroll = 1;
    ...
    case 2: /* Playfield 2 (middle layer) */
        if ((pf4_control[2] & 0x10) == 0x10) pf2_enable = 0;
        else pf2_enable = 1;
        if ((pf4_control[2] & 0x40) == 0x40) pf2_rowscroll = 1;
    ...
    case 4: /* Playfield 3 (bottom layer) */
        if ((pf4_control[4] & 0x10) == 0x10) pf3_enable = 0;
        else pf3_enable = 1;
        if ((pf4_control[4] & 0x40) == 0x40) pf3_rowscroll = 1;

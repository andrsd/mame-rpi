...
typedef struct {
    ...
    int intensity;
    int arg1;
    int arg2; /* start/end in pixel array or clipping info */
...
static void vector_pp_8(struct osd_bitmap *b, int x, int y, int p)
{
    b->line[y][x] = p;
}
static void vector_pp_16(struct osd_bitmap *b, int x, int y, int p)
{
    ((unsigned short *) b->line[y]) [x] = p;
}
static int vector_rp_8(struct osd_bitmap *b, int x, int y)
{
    return b->line[y][x];
}
static int vector_rp_16(struct osd_bitmap *b, int x, int y)
{
    return ((unsigned short *) b->line[y]) [x];
}

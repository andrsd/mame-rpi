...
WRITE_HANDLER(twin16_video_register_w)
{
    switch (offset) {
    case 0x0:
        COMBINE_WORD_MEM(&video_register, data);
        break;
    case 0x2:
        COMBINE_WORD_MEM(&scrollx[0], data);
        break;
    case 0x4:
        COMBINE_WORD_MEM(&scrolly[0], data);
        break;
    case 0x6:
        COMBINE_WORD_MEM(&scrollx[1], data);
        break;
    case 0x8:
        COMBINE_WORD_MEM(&scrolly[1], data);
        break;
    case 0xa:
        COMBINE_WORD_MEM(&scrollx[2], data);
        break;
    case 0xc:
        COMBINE_WORD_MEM(&scrolly[2], data);
        break;
                    ...
                    if (opaque) {
                        ...
                        if (bitmap->depth == 16) {
                            for (y = y1; y != y2; y += yd) {
                                ...
                                if (data) {
                                    pen = (data >> 4 * 3) & 0xf;
                                    if (pen) dest[7] = pal_data[pen];
                                    pen = (data >> 4 * 2) & 0xf;
                                    if (pen) dest[6] = pal_data[pen];
                                    pen = (data >> 4 * 1) & 0xf;
                                    if (pen) dest[5] = pal_data[pen];
                                    pen = (data >> 4 * 0) & 0xf;
                                    if (pen) dest[4] = pal_data[pen];
                                }
                                data = *gfx_data++;
                                if (data) {
                                    pen = (data >> 4 * 3) & 0xf;
                                    if (pen) dest[3] = pal_data[pen];
                                    pen = (data >> 4 * 2) & 0xf;
                                    if (pen) dest[2] = pal_data[pen];
                                    pen = (data >> 4 * 1) & 0xf;
                                    if (pen) dest[1] = pal_data[pen];
                                    pen = (data >> 4 * 0) & 0xf;
                                    if (pen) dest[0] = pal_data[pen];
                                }
                            }
                        } else {
                            for (y = y1; y != y2; y += yd) {

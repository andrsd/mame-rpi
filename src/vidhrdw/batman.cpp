...
WRITE_HANDLER(batman_playfieldram_w)
{
    ...
    if (atarigen_video_control_state.latch2 != -1)
        batman_colorram_w(offset, atarigen_video_control_state.latch2 | 0xff000000);
}
...
WRITE_HANDLER(batman_playfield2ram_w)
{
    ...
    if (atarigen_video_control_state.latch1 != -1)
        batman_colorram_w(offset, atarigen_video_control_state.latch1 | 0x00ff0000);
}
...
void batman_scanline_update(int scanline)
{
    ...
    if (scanline < YDIM && atarigen_video_control_state.rowscroll_enable) {
        UINT8 *base = &atarigen_alpharam[2 * (scanline / 8 * 64 + 48)];
        int i;

        for (i = 0; i < 8; i++, base += 4) {
            atarigen_video_control_w(0x20, READ_WORD(&base[0]));
            atarigen_video_control_w(0x20, READ_WORD(&base[2]));
...
static const UINT8 *update_palette(void)
{
    ...
    for (i = 0; i < 32; i++) {
        ...
        if (used) {
            if (i < 16)
                palette_used_colors[0x200 + i * 16 + 0] = PALETTE_COLOR_TRANSPARENT;
            else if (used & 0x0001)
                palette_used_colors[0x200 + i * 16 + 0] = PALETTE_COLOR_USED;
            for (j = 1; j < 16; j++)
                if (used & (1 << j))
...
static const UINT16 transparency_mask[4] = { 0xffff, 0xff01, 0xff01, 0x0001 };
